...
# Возвращаем твой рабочий токен
BOT_TOKEN = os.getenv("BOT_TOKEN", "8435753046:AAF2IFayV4hrAfePLcHUkUwRYNleS8FWflY") 

# Указываем правильный путь к БД для Bothost, чтобы данные не удалялись при перезагрузке сервера
...
def get_db_connection():
    # check_same_thread=False важен для работы в асинхронной среде
...
def cancel_kb(back_to_menu=True):
    ...
    if back_to_menu: rows.append([InlineKeyboardButton("🏠 Меню", callback_data="menu:home")])
...
async def send_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE, year: int, month: int):
    ...
    if inc:
        for cat, amt in inc: msg += f" • {html.escape(cat)}: {amt:.2f}\n"
    else:
        msg += " Нет записей\n"
    ...
    if exp:
        for cat, amt in exp: msg += f" • {html.escape(cat)}: {amt:.2f}\n"
    else:
        msg += " Нет записей\n"
...
async def cat_add_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_db_connection()
    try:
        conn.execute("INSERT OR IGNORE INTO categories(user_id, ttype, name, is_active, created_at) VALUES (?,?,?,1,?)", (update.message.from_user.id, context.user_data["ttype"], name, datetime.now().isoformat(timespec="seconds")))
        conn.commit()
    finally:
        conn.close()
...
async def add_confirm_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm:save":
        ...
        try:
            ...
            (q.from_user.id, context.user_data["ttype"], context.user_data["amount"], context.user_data["wallet_id"], context.user_data["category"], context.user_data.get("note"), now))
            conn.commit()
        finally:
            conn.close()
...
async def debt_pay_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_bal is None: return await on_cancel(update, context)
...
async def debt_adj_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Коммент добавлен. Сохранить корректировку?", reply_markup=kb_confirm(True))
    return DEBT_ADJ_CONFIRM
...
async def main_menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:wallets":
        ...
        return ConversationHandler.END
    if data == "w:adjust": return await adjust_entry(update, context)
    if data == "w:add": return await wallet_add_entry(update, context)
    if data == "w:archive": return await wallet_archive_entry(update, context)
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(main_menu_router, pattern=r"^(menu:|ops:|w:|stat:)"),
        ],
        states={
