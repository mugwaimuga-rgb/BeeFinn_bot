    ...
    try:
        ...
        for cat in SYSTEM_CATEGORIES:
            conn.execute(
                "INSERT OR IGNORE INTO categories(user_id,ttype,name,is_active,is_system,created_at) "
                "VALUES (?,?,?,?,1,?)",
                (user_id, "expense", cat, 1, now)
            )
            conn.execute(
                "INSERT OR IGNORE INTO categories(user_id,ttype,name,is_active,is_system,created_at) "
                "VALUES (?,?,?,?,1,?)",
                (user_id, "income", cat, 1, now)
            )
        ...
        try:
            if include_system:
                return conn.execute(
                    ...
                    "WHERE user_id=? AND ttype=? AND is_active=1 ORDER BY name",
                    (user_id, ttype)
                ).fetchall()
            return conn.execute(
                "SELECT id, name FROM categories "
                "WHERE user_id=? AND ttype=? AND is_active=1 AND is_system=0 ORDER BY name",
                (user_id, ttype)
            ).fetchall()
        finally:
            conn.close()
...
async def kb_wallets(uid: int, prefix: str, back=True):
    ...
    rows = [[InlineKeyboardButton(n, callback_data=f"{prefix}:{wid}")]
...
async def add_wallet_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "back":
        await q.edit_message_text(
            "Введи сумму:", reply_markup=cancel_kb(False)
        )
        return State.ADD_AMOUNT
...
async def add_confirm_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm:save":
        ...
        def _save():
            ...
            try:
                conn.execute(
                    "INSERT INTO transactions(user_id, ttype, amount, wallet_id, category, note, created_at) "
                    "VALUES (?,?,?,?,?,?,?)",
                    (
                        uid,
...
async def tr_confirm_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm:back":
        await q.edit_message_text(
            "Сумма перевода:", reply_markup=cancel_kb(False)
        )
        return State.TR_AMOUNT
    if q.data == "confirm:add_note":
        await q.edit_message_text("Комментарий:", reply_markup=cancel_kb(False))
            ...
            def _insert():
                ...
                try:
                    ...
                    if exists:
                        ...
                        return None
                    conn.execute(
                        "INSERT INTO transactions(user_id, ttype, amount, wallet_id, category, note, transfer_id, created_at) "
                        "VALUES (?,?,?,?,?,?,?,?)",
                        (
                            uid,
                            "expense",
...
async def send_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE, year: int, month: int):
    ...
    msg = (
        f"📊 <b>Статистика за {get_month_name(month)} {year}</b>\n"
        f"<b>🟢 Доходы:</b>\n"
    )
    ...
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("⬅ Пред.", callback_data=f"stat:{prev_y}:{prev_m}"),
            InlineKeyboardButton("След. ➡", callback_data=f"stat:{next_y}:{next_m}"),
        ],
...
async def adj_pick_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["note"] = None
    await q.edit_message_text(
        "Сколько сейчас по факту?", reply_markup=cancel_kb(False)
    )
...
async def adj_confirm_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm:back":
        await q.edit_message_text(
            "Сколько по факту?", reply_markup=cancel_kb(False)
        )
        return State.ADJ_TARGET
    if q.data == "confirm:add_note":
        await q.edit_message_text("Комментарий:", reply_markup=cancel_kb(False))
...
async def wallet_add_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Добавлен ✅", reply_markup=wallets_menu_kb()
    )
    return ConversationHandler.END
...
async def wallet_archive_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _archive():
        ...
        try:
            conn.execute(
                "UPDATE wallets SET is_active=0 WHERE user_id=? AND id=?",
    ...
    def fetch():
        ...
        try:
            my = conn.execute(
                ...
                "WHERE user_id=? AND debt_type='my_debt' AND is_active=1 AND current_balance>0",
                (uid,),
            ).fetchall()
            owed = conn.execute(
                "SELECT name, current_balance FROM debts "
                "WHERE user_id=? AND debt_type='owed_to_me' AND is_active=1 AND current_balance>0",
                (uid,),
            ).fetchall()
            return my, owed
        finally:
...
async def debt_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "debt:pay":
        ...
        if not debts:
            ...
            return State.DEBT_MENU
        rows = [
            [InlineKeyboardButton(f"{html.escape(n)} ({b:.0f})", callback_data=f"debtpay:{did}")]
    ...
    if q.data == "debt:adjust":
        ...
        if not debts:
            ...
            return State.DEBT_MENU
        rows = [
            [InlineKeyboardButton(f"{html.escape(n)} ({b:.0f})", callback_data=f"debtadj:{did}")]
            for did, n, b in debts
        ]
        rows.append([InlineKeyboardButton("❌ Отмена", callback_data="cancel")])
        await q.edit_message_text(
...
async def debt_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _save():
        ...
        try:
            conn.execute("BEGIN IMMEDIATE")
            cur = conn.execute(
                "INSERT INTO debts(user_id, debt_type, name, total_amount, current_balance, created_at) "
                "VALUES (?,?,?,?,?,?)",
                (
                    uid,
            ...
            conn.execute(
                "INSERT INTO transactions(user_id, ttype, amount, wallet_id, category, debt_id, created_at) "
                "VALUES (?,?,?,?,?,?,?)",
    ...
    try:
        await asyncio.to_thread(_save)
        await q.edit_message_text(
            "Долг оформлен ✅", reply_markup=main_menu_kb()
        )
    except Exception:
        await q.edit_message_text(
...
async def debt_pay_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        "Сумма платежа:", reply_markup=cancel_kb(False)
    )
...
async def debt_pay_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result is None:
            await q.edit_message_text(
                "Долг не найден.", reply_markup=main_menu_kb()
            )
            return ConversationHandler.END
...
async def debt_adj_target(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    old = await asyncio.to_thread(fetch_info)
    if not old:
        await update.message.reply_text(
            "Долг не найден.", reply_markup=main_menu_kb()
        )
        return ConversationHandler.END
    context.user_data.update(
        {"adj_debt_target": val, "adj_debt_old": old[0]}
    )
    delta = val - old[0]
    await update.message.reply_text(
        f"Долг: {html.escape(old[1])}\n"
        f"В базе: {old[0]:.2f}\n"
        f"По факту: {val:.2f}\n"
        f"Разница: {delta:.2f}\n\n"
        f"Сохранить?",
...
async def debt_adj_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["note"] = update.message.text.strip() or None
    await update.message.reply_text(
        "Коммент добавлен. Сохранить?", reply_markup=kb_confirm(True)
    )
    return State.DEBT_ADJ_CONFIRM
...
async def debt_adj_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "confirm:back":
        await q.edit_message_text(
            "Фактический остаток:", reply_markup=cancel_kb(False)
        )
        return State.DEBT_ADJ_TARGET
    if q.data == "confirm:add_note":
        await q.edit_message_text("Комментарий:", reply_markup=cancel_kb(False))
...
async def main_menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:ops":
        await q.edit_message_text(
            "Выберите операцию:", reply_markup=ops_menu_kb()
        )
        return ConversationHandler.END

    if data in ("ops:income", "ops:expense"):
        return await add_entry(
            update, context, "income" if data == "ops:income" else "expense"
        )
    ...
    if data == "menu:wallets":
        ...
        async def calc():
            ...
            for wid, n, act in wallets:
                ...
                balances.append((wid, n, act, bal))
            return balances

        balances = await calc()
        active = [(n, b) for _, n, act, b in balances if act]
        total = sum(b for _, b in active)
        lines = [
            f"{html.escape(n)}: {b:.2f}" for n, b in active
        ] + [f"\n<b>Итого: {total:.2f}</b>"]
        await q.edit_message_text(
            "\n".join(lines),
            reply_markup=wallets_menu_kb(),
...
def main():
    ...
    # Глобальный обработчик кнопок главного меню/верхнего уровня
    app.add_handler(
    ...
    # ConversationHandler только для пошаговых сценариев
    conv = ConversationHandler(
        entry_points=[],  # сюда НЕ попадает main_menu_router
        states={
            State.ADD_AMOUNT: [
            ...
            State.TR_NOTE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, tr_note)
            ],
            ...
            State.ADJ_NOTE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, adj_note)
            ],
